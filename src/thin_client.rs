...
impl ThinClient {
        ...
        thread::spawn(move || loop {
            ...
            match receiver.try_recv() {
                Ok(tuple) => {
                    ...
                    Self::process_message(client_addr, message, tx)
                        .expect("thin client process message");
                }
    ...
    fn process_message(
        mut client_addr: ThinClientAddr,
        message: Message,
        response_sender: Sender<Response>,
    ) -> io::Result<usize> {
        match message {
            Message::Requests(request) => {
                let resp = client_addr
                    .poll_for_data(request)
                    .expect("transaction_count response");
                // Need error resp (timeout) handling here
                info!("recv_response {:?}", resp);
                response_sender
                    .send(resp)
                    .expect("send response from thread");
                Ok(0) // CLEAN THIS UP
            }
            Message::Transactions(tx) => client_addr.send_event_data(tx),
        }
    }
    ...
    pub fn transfer_signed(&self, tx: Transaction) -> io::Result<()> {
        ...
        self.sender
            .send((Message::Transactions(tx), response_sender))
            .expect("send request to thread");
        Ok(()) // CLEAN THIS UP
    }
    ...
    pub fn get_balance(&mut self, pubkey: &PublicKey) -> io::Result<i64> {
        ...
        self.sender
            .send((Message::Requests(req), response_sender))
            .expect("send request to thread");
    ...
    pub fn poll_for_data(&mut self, req: Request) -> io::Result<Response> {
        ...
        loop {
            ...
            for event in events.iter() {
                ...
                match event.token() {
                    SENDER => {
                        ...
                        let bytes_sent =
                            requests_socket.send_to(&data, &self.requests_server_addr)?;
                    ...
                    RECEIVER => {
                        ...
                        let resp: Response =
                            deserialize(&buf).expect("deserialize balance in thin_client");
            ...
            if events.is_empty() {
                ...
                if num_tries > 0 {
                    debug!(
                        "thin_client retry poll for data, {:?} attempts remaining",
                        num_tries
                    );
                    num_tries -= 1;
                    ...
                    let custom_error =
                        io::Error::new(io::ErrorKind::TimedOut, "thin client receive timed out");
            ...
            for event in events.iter() {
                match event.token() {
                    SENDER => {
                        ...
                        let bytes_sent =
                            transactions_socket.send_to(&data, &self.transactions_server_addr)?;
                        trace!("{:?} bytes sent", bytes_sent);
